...
def main(args):
    ...
    if args['weights'] is not None:
        ...
        if any(k.startswith("module.") for k in ckpt_state_dict.keys()):
            ckpt_state_dict = {k.replace("module.", ""): v for k, v in ckpt_state_dict.items()}
        
        # Get the number of classes from the loaded checkpoint.
        if 'roi_heads.box_predictor.cls_score.weight' in ckpt_state_dict:
            old_classes = ckpt_state_dict['roi_heads.box_predictor.cls_score.weight'].shape[0]
        else:
            raise KeyError("'roi_heads.box_predictor.cls_score.weight' not found in checkpoint. "
                        "Ensure the checkpoint matches the model architecture.")
        
        # Build the new model with the same number of classes as the checkpoint.
        build_model = create_model[args['model']]
        ...
        # Update the predictor layers for the current number of classes
